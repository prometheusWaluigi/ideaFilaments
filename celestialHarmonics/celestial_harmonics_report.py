...
"""
    ...
    def _format_patterns(self):
        ...
        for pattern_type, pattern_list in self.patterns.items():
            if pattern_list:
                pattern_sections.append(f"#### {pattern_type.replace('_', ' ').title()} Patterns")
                for pattern in pattern_list:
                    pattern_sections.append(f"- **Resonance**: {pattern.get('resonance', 'N/A')}")
                    pattern_sections.append(f"  - **Points**: {', '.join(pattern.get('points_involved', []))}")
        return "\n".join(pattern_sections) if pattern_sections else "No significant patterns detected."
    ...
    def generate_visualization(self, output_path='celestial_harmonics_plot.png'):
        ...
        for p in self.points:
            plt.annotate(p.name, (p.ra_degrees, p.dec_degrees), 
